...
import { getEventStatus, formatDatePtBr } from "@/lib/utils";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { getUserRequestForEvent } from "@/lib/api/requests";
...
export default function EventTitle({
  ...
  useEffect(() => {
    async function load() {
      const res = await getUserRequestForEvent(event.uuid);
      setHasRequest(!!res);
    }
    load();
  }, [event.uuid]);
  ...
  return (
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b pb-8">
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 text-muted-foreground text-sm">
          <div className="flex items-center gap-1.5">
            ...
            <span>
              {formatDatePtBr(event.start_date)} até {formatDatePtBr(event.end_date)}
            </span>
          </div>
      ...
      {/* Caso já tenha request */}
      {!isAdmin && hasRequest === true && (
        <div>
